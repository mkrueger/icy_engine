...
fn test_linebreak_bug() {
    ...
    assert_eq!('\x16', buf.get_char(Position {x: 1, y: 0}).unwrap_or_default().ch);
}
...
fn test_reset_empty_colors() {
    ...
    assert_eq!(buf.get_char(Position::new(0, 0)).unwrap().attribute, buf.get_char(Position::new(2, 0)).unwrap().attribute);
    assert_eq!(buf.get_char(Position::new(1, 0)).unwrap().attribute, buf.get_char(Position::new(5, 0)).unwrap().attribute);
    assert_eq!(buf.get_char(Position::new(2, 0)).unwrap().attribute, buf.get_char(Position::new(8, 0)).unwrap().attribute);
}
