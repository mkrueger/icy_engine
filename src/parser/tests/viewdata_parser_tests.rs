use crate::{Buffer, BufferParser, Caret, Position, ViewdataParser};
...
fn testpage_bug_3() {
    ...
    let (buf, _) =
        create_viewdata_buffer(&mut ViewdataParser::new(), b"\x1BM \x1BE\x1B]\x1BBT\x1B\\X");
    assert_eq!(
        0,
