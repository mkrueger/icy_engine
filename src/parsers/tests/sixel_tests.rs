...
fn test_overwrite_multiple_sixels() {
    let (mut buf, mut caret) = create_buffer(&mut AnsiParser::new(), b"\x1B[0;0H\x1BPq#0;2;0;0;0#1;2;100;100;0#2;2;0;100;0#1~~@@vv@@~~@@~~$43#2??}}GG}}??}}??-#1!14@\x1B\\");
    update_buffer(&mut buf, &mut caret, &mut AnsiParser::new(), b"\x1B[5;5H\x1BPq#0;2;0;0;0#1;2;100;100;0#2;2;0;100;0#1~~@@vv@@~~@@~~$43#2??}}GG}}??}}??-#1!14@\x1B\\");
    update_buffer(&mut buf, &mut caret, &mut AnsiParser::new(), b"\x1B[10;10H\x1BPq#0;2;0;0;0#1;2;100;100;0#2;2;0;100;0#1~~@@vv@@~~@@~~$43#2??}}GG}}??}}??-#1!14@\x1B\\");

    for _ in 0..10 {
        update_buffer(&mut buf, &mut caret, &mut AnsiParser::new(), b"\x1B[0;0H\x1BPq#0;2;0;0;0#1;2;100;100;0#2;2;0;100;0#1~~@@vv@@~~@@~~$43#2??}}GG}}??}}??-#1!14@\x1B\\");
        update_buffer(&mut buf, &mut caret, &mut AnsiParser::new(), b"\x1B[5;5H\x1BPq#0;2;0;0;0#1;2;100;100;0#2;2;0;100;0#1~~@@vv@@~~@@~~$43#2??}}GG}}??}}??-#1!14@\x1B\\");
        update_buffer(&mut buf, &mut caret, &mut AnsiParser::new(), b"\x1B[10;10H\x1BPq#0;2;0;0;0#1;2;100;100;0#2;2;0;100;0#1~~@@vv@@~~@@~~$43#2??}}GG}}??}}??-#1!14@\x1B\\");
    }

    assert_eq!(3, buf.layers[0].sixels.len());
    assert_eq!(Position::new(0, 0), buf.layers[0].sixels[0].position);
    assert_eq!(Position::new(4, 4), buf.layers[0].sixels[1].position);
    assert_eq!(Position::new(9, 9), buf.layers[0].sixels[2].position);

    for i in 0..buf.layers[0].sixels.len() {
        assert_eq!(14, buf.layers[0].sixels[i].width());
        assert_eq!(12, buf.layers[0].sixels[i].height());
    }
}
...
fn test_sixel_raster_attributes() {
    let (buf, _) = create_buffer(&mut AnsiParser::new(), b"\x1BPq\"2;3;6;8\x1B\\");
    assert_eq!(1, buf.layers[0].sixels.len());
